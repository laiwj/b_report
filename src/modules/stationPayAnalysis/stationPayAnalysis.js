...
var vm1 = avalon.define({
    ...
    getChartsData: function() {
        ...
        if (_model.name === '请选择') {
            _model.name = '产品经理';
            vm1.configItem.name = "产品经理";
        }
        if (!_model.name) {
            avalon.vmodels.root.showMessage('请选择职位！');
            return false;
        }
        ...
        var url = '/api/position/salary/analysis';
    ...
    reportCollect: function() { //收藏报告
        ...
        _model.industry = _model.industry === '请选择' ? '' : _model.industry;
        _model.city = _model.city === '请选择' ? '' : _model.city;
        _collect.report_name = _model.name + _model.industry + _model.city + _model.experience + 'Top5岗位薪酬分析报告';
        collect(_collect);
    }
})
